...
script = '{base_dir}/var/log_collect.sh'.format(base_dir=base_dir)
...
def generate_script():
    ...
    with open(script,'w') as e:
        ...
        for i in file_lst:
            line="awk -F \" \" '{if ($1==\"%s\" &&   $2>=\"%s\" && $2<=\"%s\" ) print $0}' %s >>%s"%(date,time_start,time_stop,i,result_txt)
            e.write(line+'\n')
...
def httpd_src():
    ...
    with open(script, 'a') as e:
        for i in logfile_lst2:
            line1 = "cat {log}| egrep '{date}/{month}/2017:{times1}' >>{result_txt}".format(log=i, date=day, month=month, times1=times1, times2=times2,result_txt=result_txt)
            line2 = "cat {log}| egrep '{date}/{month}/2017:{times2}' >>{result_txt}".format(log=i, date=day, month=month, times1=times1, times2=times2,result_txt=result_txt)
            e.write(line1 + '\n' + line2 + '\n')
...
def rabbitmq_src():
    ...
    with open(script,'a') as e:
        line1 = "cat {rabbitmq_log}| egrep '{date}-{month}-2017::{times1}' >>{result_txt}".format(rabbitmq_log=rabbitmq_log,date=day,month=month,times1=times1,times2=times2,result_txt=result_txt)
        line2 = "cat {rabbitmq_log}| egrep '{date}-{month}-2017::{times2}' >>{result_txt}".format(rabbitmq_log=rabbitmq_log,date=day,month=month,times1=times1,times2=times2,result_txt=result_txt)
        e.write(line1 + '\n'+line2 + '\n')
        e.write('sed -i "s/{} {}/{}/" {} \n'.format(month,day,date,result_txt))
